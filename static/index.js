...
// src/game.ts
var getNextDirectionClockwise = (dir, steps = 1) => {
  const directions = [
    "UP" /* UP */,
    "RIGHT" /* RIGHT */,
    "DOWN" /* DOWN */,
...
class Position {
  ...
  isWithinBounds(bounds) {
    return this.x >= 0 && this.x < bounds.width && this.y >= 0 && this.y < bounds.height;
  }
}
...
class Snake {
  ...
  facing;
  segments;
  constructor(id, segments, facing, color, name) {
    this.id = id;
    this.color = color;
    this.facing = facing;
  ...
  name;
  static fromSpawnPosition(id, pos, length, facing, color, name) {
    const segments = [];
...
class Game {
    ...
    const directionsByDescendingFreeness = directions.sort((a, b) => {
      const countFree = (dir) => {
        ...
        while (!isBlocked) {
          ...
          isBlocked = this.players.some((j) => j.overlapsWith(newPos)) || !newPos.isWithinBounds({ width: this.width, height: this.height });
          if (!isBlocked) {
            freeTiles += 1;
          }
        }
  ...
  getRandomFreePos() {
    let pos = null;
    while (!pos || this.players.some((i) => i.overlapsWith(pos)) || this.tiles.some((i) => i.pos.equals(pos))) {
  ...
  tick() {
    ...
    for (const player of this.players) {
      ...
      const collisions = this.tiles.filter((i) => i.pos.equals(player.head.pos));
  ...
  playerMovedThisTick = {};
  setPlayerFacing(playerId, direction) {
    const player = this.players.find((i) => i.id === playerId);
    if (!player)
      return;
    const alreadyMoved = this.playerMovedThisTick[playerId] === true;
    if (!alreadyMoved && direction !== player.facing && player.canFace(direction)) {
...
class GameState {
  ...
  static fromJSON(data) {
    ...
    state.players = data.snakes.map((i) => {
      return new Snake(i.id, i.pos.map((j) => new SnakeSegment(new Position(j[0], j[1]))), i.facing, i.color, i.name);
    });
...
class GameClient {
  ...
  lastPing = null;
  getPing() {
    if (this.lastPing != null && performance.now() - this.lastPing.time < 5000) {
...
async function getSocket(token) {
  const socket = lookup2(SERVER_URL, {
...
class Renderer {
  ...
  get scale() {
    return Math.min(this.canvas.width / this.game.state.size.width, this.canvas.height / this.game.state.size.height);
  }
  init() {
    this.game.events.on("tick", this.render.bind(this));
  }
  setColor(color, bloom = true) {
  ...
  async render() {
    ...
    if (this.debug) {
      ...
      this.ctx.fillRect(0, 0, this.game.state.size.width * this.scale, this.game.state.size.height * this.scale);
    } else {
    ...
    for (const tile of this.game.state.tiles) {
      this.setColor("red");
      this.ctx.fillRect(tile.pos.x * this.scale, tile.pos.y * this.scale, this.scale, this.scale);
    }
...
// src/client/index.ts
async function getHighscores(token) {
